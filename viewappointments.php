...
<body>
	...
	<table class="table4">
		...
		<?php $sql3="SELECT  * FROM  patientappo " ;
		$result3=$conn->query($sql3);
		if(mysqli_num_rows($result3)>=1){
			while ($row3=$result3->fetch_assoc()) {

				echo "<tr><td>".$row3["app_ID"]."</td><td>".$row3["den_name"]."</td><td>".$row3["pID"]."</td><td>".$row3["App_date"]."</td><td>".$row3["App_time"]."</td><td>".$row3['App_cause']."</td></tr>";
			}
    ...
    <form action = "confirmationEmail.php" role="form" method="post" enctype="multipart/form-data" class="form1" style="width: 100%;margin-top:110px;margin-left: -10px;">

    
    <div classs="xa" style="margin-top:0px">
        <label for="email" style="width:40px;">Email</label>
        <input type="email"  id="email" name="email" maxlength="50" style="width:400px;" placeholder="xyz@gmail.com">
        ...
        <button type="submit"  id="message" name="submit" style="width:200px;height:30px;border-radius: 10px; background: #992828 ;margin-left:45%;color: white ">Send
               </button>
    ...
    if (isset($_POST['sendmail'])){
        if(mail($_POST['email'], $_POST['subject'], $_POST['message'],'FROM:eesha.qureshi723@gmail.com')){
            echo "success";
        }else{
            echo "failed";
        }
